...
- [`formAction`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input#formaction): Merkkijono tai funktio. Ylikirjoittaa ylätason `<form action>` kentille joissa `type="submit"` tai `type="image"`. Jos URL välitetään `action`:lle se käyttäytyy kuin standardi HTML lomake. Jos funktio välitetään `action`:lle, funktio suorittaa lomakkeen lähettämisen. Katso [`<form action>`](/reference/react-dom/components/form#props).
